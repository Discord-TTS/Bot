...
use crate::{require, structs::{FrameworkContext, Data}, traits::PoiseContextExt, Context, OptionTryUnwrap, constants};
...
pub async fn handle_unexpected<'a>(
    ...
    ").bind(traceback_hash.clone()).fetch_optional(&mut *conn).await? {
        ...
        let shard_count = poise_context.shard_manager.shards_instantiated().await.len();
        let after_fields = [
...
pub async fn handle_unexpected_default(ctx: &serenity::Context, framework: FrameworkContext<'_>, name: &str, result: Result<()>) -> Result<()> {
...
pub async fn handle_member(ctx: &serenity::Context, framework: FrameworkContext<'_>, member: &serenity::Member, result: Result<(), impl Into<Error>>) -> Result<()> {
...
pub async fn handle_guild(name: &str, ctx: &serenity::Context, framework: FrameworkContext<'_>, guild: Option<&serenity::Guild>, result: Result<()>) -> Result<()> {
    let error = require!(result.err(), Ok(()));
    ...
    match error {
        ...
        poise::FrameworkError::Command { error, ctx, .. } => {
            ...
            let mut extra_fields = vec![
                ...
                ("Channel Type", Cow::Borrowed(channel_type(&ctx.channel_id().to_channel(&ctx).await?)), true),
            ];
            ...
            handle_unexpected(
                ctx.serenity_context(), ctx.framework(),
...
pub async fn interaction_create(ctx: &serenity::Context, interaction: &serenity::Interaction, framework: FrameworkContext<'_>) -> Result<(), Error> {
...
pub async fn handle_traceback_button(ctx: &serenity::Context, data: &Data, interaction: &serenity::ComponentInteraction) -> Result<(), Error> {
