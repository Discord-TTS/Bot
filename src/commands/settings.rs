...
use crate::database;
use crate::funcs::{confirm_dialog, current_user_id, random_footer};
use crate::structs::{
    ApplicationContext, Command, CommandResult, Context, Data, Error, PollyVoice, Result,
    SpeakingRateInfo, TTSMode, TTSModeChoice,
};
    ...
    let voices: &mut dyn Iterator<Item = _> = match mode {
        ...
        TTSMode::Polly => {
            i3 = ctx
                ...
                .polly_voices
                .values()
                .map(|voice| poise::AutocompleteChoice {
                    name: format!(
                        "{} - {} ({})",
                        voice.name, voice.language_name, voice.gender
                    ),
        ...
        TTSMode::gCloud => {
            i4 = ctx
                ...
                .flat_map(|(language, variants)| {
                    variants
                        .iter()
                        .map(move |(variant, gender)| poise::AutocompleteChoice {
...
pub async fn nick(
    ...
    if author.id != user.id
        && !guild_id
            .member(ctx_discord, author)
            .await?
            .permissions(ctx_discord)?
            .administrator()
    {
