    ...
    if message.embeds and message.channel.id == 749971061843558440 and str(message.author) == "GitHub#0000":
      ...
      if say:
        if message.author.id not in self.bot.blocked_users:
          tosend = f"{str(message.author)} dmed {self.bot.user.mention}: {message.content}"
          if message.attachments:
             tosend = f"{tosend} and attached: {[attachment.url for attachment in message.attachments]}"
          await self.bot.channels["dm_logs"].send(tosend)
  ...
  async def on_member_join(self, member):
    if member is not None:
      if member.guild.owner == member:
        if guild.owner in self.bot.supportserver.members:
