...
class events_errors(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, (commands.BadArgument, commands.MissingRequiredArgument, commands.UnexpectedQuoteError, commands.ExpectedClosingQuoteError)):
            return await ctx.send(f"Did you type the command right, {ctx.author.mention}? Try doing l-help!")
            ...
            return await ctx.author.send("Unknown Permission Error, please give TTS Bot the required permissions. If you want this bug fixed, please do `l-suggest *what command you just run*`")
